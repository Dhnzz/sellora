        ...
        <div class="card-body">
            <div class="d-flex justify-content-end align-items-start g-2 w-full">
                <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#modalTambah">
                    <i class="ti ti-plus"></i> &nbsp; Tambah unit produk
                </button>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modalTambahLabel">Tambah Unit Produk</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('owner.master_data.product_unit.store') }}" method="POST"
    ...
    <script>
        ...
        $(document).ready(function() {
            dataTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{ route('owner.master_data.product_unit.data') }}", // Endpoint API untuk DataTables
                    type: 'GET',
            ...
            $(document).on('click', '.delete-btn', function(e) {
                ...
                var productUnitId = $(this).data('id');
                var deleteUrl = "{{ route('owner.master_data.product_unit.destroy', ':id') }}";
                deleteUrl = deleteUrl.replace(':id', productUnitId);

                if (confirm(
                        'Apakah Anda yakin ingin menghapus unit produk ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: deleteUrl,
                        ...
                        error: function(xhr) {
                            ...
                            toastr.error('Terjadi kesalahan saat menghapus unit produk.');
                        }
                    });
                }
            })
            ...
            $(document).on('click', '.edit-btn', function(e) {
                var id = $(this).data('id');
                var editUrl = "{{ route('owner.master_data.product_unit.edit', ':id') }}";
                ...
                var updateUrl = "{{ route('owner.master_data.product_unit.update', ':id') }}/";
                ...
                $.ajax({
                    ...
                    type: 'GET',
                    success: function(response) {
                        if (response.productUnit) {
                            form.find('[name="name"]').val(response.productUnit.name);
