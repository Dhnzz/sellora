                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="mb-3">
                            ...
                            <small>*Tombol reset akan mengubah password menjadi : admin123</small>
                            @error('password')
                                <small class="invalid-feedback">
                                    {{ $message }}
                                </small>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="address" class="form-control" rows="4">{{ $admin->address }}</textarea>
                        </div>
                        <div class="mb-3 row row-cols-1 row-cols-md-2 justify-content-md-between">
                            <div class="col mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-danger delete-photo-btn" data-id="{{ $admin->id }}"
                            ...
                            <div class="col-12 col-md-6 py-auto">
                                ...
                                @else
                                    <img src="{{ asset('storage/' . $admin->photo) }}" class="img-fluid rounded w-50"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.reset-pass-btn', function(e) {
                ...
                var resetPassUrl = "{{ route('owner.user_management.admin.resetPassword', ':id') }}";
                resetPassUrl = resetPassUrl.replace(':id', adminId);

                if (confirm(
                        'Apakah Anda yakin ingin mereset password admin ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: resetPassUrl,
                        ...
                        error: function(xhr) {
                            ...
                            toastr.error('Terjadi kesalahan saat mereset password admin.');
                        }
                    });
                }
            })
            ...
            $(document).on('click', '.delete-photo-btn', function(e) {
                ...
                var adminId = $(this).data('id');
                var deletePhotoUrl = "{{ route('owner.user_management.admin.deletePhoto', ':id') }}";
                deletePhotoUrl = deletePhotoUrl.replace(':id', adminId);

                if (confirm(
                        'Apakah Anda yakin ingin menghapus foto admin ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: deletePhotoUrl,
                        type: 'PUT', // Menggunakan metode HTTP DELETE
                        ...
                        error: function(xhr) {
                            console.error('AJAX Error:', xhr.responseText);
                            toastr.error('Terjadi kesalahan saat menghapus foto admin.');
                        }
                    });
                }
            })
