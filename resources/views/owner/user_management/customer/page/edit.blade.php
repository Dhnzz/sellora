...
@section('content')
    ...
    <div class="card">
        <form action="{{ route('owner.user_management.customer.update', $customer->id) }}" method="post"
            enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="card-body">
                <a href="{{ route('owner.user_management.customer.index') }}" class="btn btn-sm btn-primary mb-3"><i class="ti ti-arrow-left"></i>
                    Kembali</a>
                <div class="row">
                    <div class="col-12">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <button class="btn btn-primary reset-pass-btn" data-id="{{ $customer->id }}"
                                    type="button"><i class="ti ti-restore"></i></button>
                            </div>
                            <small>*Tombol reset akan mengubah password menjadi : customer123</small>
                            @error('password')
                                <small class="invalid-feedback">
                                    {{ $message }}
                                </small>
                        ...
                        <div class="mb-3 row row-cols-1 row-cols-md-2 justify-content-md-between">
                            <div class="col mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-danger delete-photo-btn" data-id="{{ $customer->id }}"
                            ...
                            <div class="col-12 col-md-6 py-auto">
                                ...
                                @else
                                    <img src="{{ asset('storage/' . $customer->photo) }}" class="img-fluid rounded w-50"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.reset-pass-btn', function(e) {
                ...
                var resetPassUrl = "{{ route('owner.user_management.customer.resetPassword', ':id') }}";
                resetPassUrl = resetPassUrl.replace(':id', customerId);

                if (confirm(
                        'Apakah Anda yakin ingin mereset password customer ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: resetPassUrl,
                        ...
                        error: function(xhr) {
                            ...
                            toastr.error('Terjadi kesalahan saat mereset password customer.');
                        }
                    });
                }
            })
            ...
            $(document).on('click', '.delete-photo-btn', function(e) {
                ...
                var customerId = $(this).data('id');
                var deletePhotoUrl = "{{ route('owner.user_management.customer.deletePhoto', ':id') }}";
                deletePhotoUrl = deletePhotoUrl.replace(':id', customerId);

                if (confirm(
                        'Apakah Anda yakin ingin menghapus foto customer ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: deletePhotoUrl,
                        type: 'PUT', // Menggunakan metode HTTP DELETE
                        ...
                        error: function(xhr) {
                            console.error('AJAX Error:', xhr.responseText);
                            toastr.error('Terjadi kesalahan saat menghapus foto customer.');
                        }
                    });
                }
            })
