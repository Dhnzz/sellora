...
@section('content')
    ...
    <div class="card">
        <form action="{{ route('owner.user_management.sales.update', $sales->id) }}" method="post"
                    ...
                    <div class="col-12">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <button class="btn btn-primary reset-pass-btn" data-id="{{ $sales->id }}"
                                    type="button"><i class="ti ti-restore"></i></button>
                            </div>
                            <small>*Tombol reset akan mengubah password menjadi : sales_agent123</small>
                            @error('password')
                                <small class="invalid-feedback">
                                    {{ $message }}
                                </small>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="address" class="form-control" rows="4">{{ $sales->address }}</textarea>
                        </div>
                        <div class="mb-3 row row-cols-1 row-cols-md-2 justify-content-md-between">
                            <div class="col mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-danger delete-photo-btn" data-id="{{ $sales->id }}"
                            ...
                            <div class="col-12 col-md-6 py-auto">
                                ...
                                @else
                                    <img src="{{ asset('storage/' . $sales->photo) }}" class="img-fluid rounded w-50"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.reset-pass-btn', function(e) {
                ...
                var resetPassUrl = "{{ route('owner.user_management.sales.resetPassword', ':id') }}";
                resetPassUrl = resetPassUrl.replace(':id', salesId);

                if (confirm(
                        'Apakah Anda yakin ingin mereset password agen sales ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: resetPassUrl,
                        ...
                        error: function(xhr) {
                            ...
                            toastr.error('Terjadi kesalahan saat mereset password agen sales.');
                        }
                    });
                }
            })
            ...
            $(document).on('click', '.delete-photo-btn', function(e) {
                ...
                var salesId = $(this).data('id');
                var deletePhotoUrl = "{{ route('owner.user_management.sales.deletePhoto', ':id') }}";
                deletePhotoUrl = deletePhotoUrl.replace(':id', salesId);

                if (confirm(
                        'Apakah Anda yakin ingin menghapus foto agen sales ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: deletePhotoUrl,
                        type: 'PUT', // Menggunakan metode HTTP DELETE
                        ...
                        error: function(xhr) {
                            console.error('AJAX Error:', xhr.responseText);
                            toastr.error('Terjadi kesalahan saat menghapus foto agen sales.');
                        }
                    });
                }
            })
