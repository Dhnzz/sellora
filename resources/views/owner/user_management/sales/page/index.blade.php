    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-end align-items-start g-2 w-full">
                <a href="{{ route('owner.user_management.sales.create') }}" class="btn btn-sm btn-success">
                    <i class="ti ti-plus"></i> &nbsp; Tambah Agen Sales
                </a>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            dataTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{ route('owner.user_management.sales.data') }}", // Endpoint API untuk DataTables
                    type: 'GET',
            ...
            $(document).on('click', '.delete-btn', function(e) {
                ...
                var deleteUrl = "{{ route('owner.user_management.sales.destroy', ':id') }}";
                deleteUrl = deleteUrl.replace(':id', salesAgentId);

                if (confirm(
                        'Apakah Anda yakin ingin menghapus agen sales ini? Tindakan ini tidak dapat dibatalkan!'
                    )) {
                    $.ajax({
                        url: deleteUrl,
                        ...
                        error: function(xhr) {
                            ...
                            toastr.error('Terjadi kesalahan saat menghapus agen sales.');
                        }
                    });
                }
            })
